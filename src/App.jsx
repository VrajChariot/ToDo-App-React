        ...
        <div className='w-full h-[1px] bg-violet-600 opacity-80 my-2'></div>
        {
          todos.map(item => {
            return (
              ...
              <div className='flex items-center gap-2'>
                <button className='bg-red-500 text-white rounded-full px-3 py-3' onClick={() => handleDeleteTodo(item.id)}><MdDeleteOutline /></button>
                <button className='bg-violet-600 text-white rounded-full px-3 py-3' onClick={() => handleEditTodo(item.id)}><RiEdit2Fill /></button>
              </div>
            </div>
            )
          })
